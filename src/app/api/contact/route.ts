    ...
    if (process.env.RESEND_API_KEY) {
      ...
      if (notifyOwner) {
        try {
          ...
          // The lead is already saved in Convex, so a returned email error is non-fatal. Log loudly.
          if (ownerEmailError) {
            console.error('[Contact API] Owner notification email failed:', ownerEmailError);
          }
        } catch (ownerEmailThrew) {
          // A thrown transport error must stay non-fatal too, or the customer retries and duplicates the lead.
          console.error('[Contact API] Owner notification email threw:', ownerEmailThrew);
        }
      }
      ...
      try {
          ...
          html: `
            <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #070707;">
              ...
              <div style="padding: 2rem;">
                ...
                <p style="font-size: 1rem; line-height: 1.6;">Thank you for your interest in Melting Moments Catering. We have received your inquiry. Chef Paul will personally reply within one business day.</p>
        ...
        // A thrown transport error must stay non-fatal too, or the customer retries and duplicates the lead.
        console.error('[Contact API] Customer confirmation email threw:', confirmationEmailThrew);
      }
    }
