  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div 
          role="listbox"
          style={{ position: 'absolute', top: '100%', left: 0, width: '100%', backgroundColor: 'white', border: '1px solid rgba(0,0,0,0.1)', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', zIndex: 50, animation: 'fadeIn 0.2s ease', marginTop: '4px' }}
        >
          {options.map((opt, idx) => (
            <div 
              key={opt.value}
                ...
                {step === 2 && (
                    <form onSubmit={handleNext} style={{ animation: 'fadeIn 0.5s ease forwards' }}>
                        ...
                        <label style={{ display: 'block', marginBottom: '1.5rem' }}>
                            ...
                            <input 
                                type="text"
                                placeholder="e.g. Victoria Conference Centre"
